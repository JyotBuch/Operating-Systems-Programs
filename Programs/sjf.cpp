...
void sort_by_BT(process proc[],int n)
{
//sorts processes by Burst Time from index 1
    process temp;
    for(int i = 0; i < n - 1; i++)
    {
...
{
sort_by_AT(proc,n);
sort_by_BT(proc,n); //sorts from index 1 so 1st element is still the one with least arrival time
