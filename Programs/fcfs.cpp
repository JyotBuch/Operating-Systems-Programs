...
void FCFS(process proc[],int n)
{
    ...
    for(int i = 1; i < n; i++)
    {
        //Calculate idle time
        idleTime = 0;
        if(proc[i-1].finishTime<proc[i].arrivalTime)
        {
            idleTime = proc[i].arrivalTime-proc[i-1].finishTime;
        }

        //Calculate Completion time, turnaround time, waiting time
        proc[i].finishTime = proc[i - 1].finishTime + proc[i].burstTime + idleTime;
